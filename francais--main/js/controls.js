...
const Controls = {
    ...
    _onKeyDown(e) {
        if (this.mediaOverlayClose && (e.code === 'Escape' || e.code === 'KeyF' || e.code === 'KeyE')) {
            e.preventDefault();
            this.mediaOverlayClose();
            return;
        }
    ...
    /**
     * Handle interaction with an object (click or E key)
     */
    _onInteract(obj) {
        if (!obj.userData.interactive) return;
        if (obj.userData.mediaType && obj.userData.mediaSrc) {
            this._openMediaOverlay(obj.userData);
            return;
        }
        if (typeof obj.userData.onInteract === 'function') {
    ...
    _openMediaOverlay(data) {
        if (this.isLocked) {
            document.exitPointerLock();
        }

        const existing = document.getElementById('media-overlay');
        if (existing) {
            existing.remove();
        }
        ...
        if (data.mediaType === 'video') {
            ...
            video.className = 'media-overlay-video';
            video.play().catch(() => {});
            content.appendChild(video);

            video.addEventListener('error', () => {
                const hint = document.createElement('div');
                hint.className = 'media-overlay-hint';
                hint.textContent = 'Vidéo non lisible sur ce navigateur. Vérifiez le codec MP4.';
            ...
            img.className = 'media-overlay-image';
        ...
        this.mediaOverlayClose = closeOverlay;
    },
    ...
    _openPrototypeModal(obj) {
        ...
        if (data.prototypeFunc) {
            ...
            sourceObj = data.modelToClone || obj;
            if (sourceObj) {
                prototypeObj = sourceObj.clone();
                prototypeObj.position.set(0, 0, 0);
                prototypeObj.rotation.set(0, 0, 0);
                prototypeObj.visible = true;
                
                // Recursively ensure visibility
                prototypeObj.traverse((child) => {
                    if (child.isMesh) {
                        child.visible = true;
                    }
                });
